...
//  --Generated On Tue Jun 09 18:15:31 IST 2020******************* 
//  **************** Start jsonWriter.js*******************
//#ifdef iphone
	//#define KONYSYNC_IOS
//#endif
...
kony.sync.validateScopeSession = function(abortSync, syncErrorObject) {
	...
	if(kony.sync.isSyncStopped){
		sync.log.debug("sync stopped after function validateScopeSession");
		kony.sync.stopSyncSession();
		kony.sync.isSessionInProgress = false;
		kony.sync.httprequestsinglesession = false;
		return;
...
kony.sync.stopSyncSession = function(){
	...
	try {
		if (kony.sync.isSyncStoppedDueToNetworkFailure) {
			kony.sync.verifyAndCallClosure(kony.sync.currentSyncConfigParams[kony.sync.onSyncError],
										   kony.sync.getErrorTable(kony.sync.errorCodeSyncError,
...
kony.sync.initializeChunkMapper = function(chunkSize) {
	if (chunkSize == null && typeof chunkSize != 'number') {
		sync.log.warn('inside kony.sync.initializeChunkMapper , found chunkSize not to be a number datatype');
		throw new Error('chunk size is not a number');
	}

	if (kony.sync.chunkMapperInstance != null) {
		kony.sync.chunkMapperInstance.destroy();
		kony.sync.chunkMapperInstance = null;
	}
	sync.log.info('inside kony.sync.initializeChunkMapper , initializing with chunkSize -' + chunkSize);
	kony.sync.chunkMapperInstance = new kony.sync.chunkMapper();
	kony.sync.chunkMapperInstance.initialize(chunkSize);
};

kony.sync.chunkMapper = function() {
	var chunkMap = null;

	function initialize(chunkNumber) {
		chunkMap = {};
		for (var i = 1; i <= chunkNumber; i++) {
			chunkMap[i] = kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_NOT_INITIATED;
		}
		return chunkMap;
	};

	function setChunkStatus(chunkNumber, status) {
		var setStatus = false;
		if (isInitialized()) {
			chunkMap[chunkNumber] = status;
			setStatus = true;
		}
		return setStatus;
	};
	...
	function getChunkStatus(chunkNumber) {
		var status = -1;
		if (isInitialized()) {
			status = chunkMap[chunkNumber];
		}
		return status;
	}

	function destroyChunkMap() {
		chunkMap = null;
	}

	function printStatus() {
		if (kony.logger.currentLogLevel <= kony.logger.logLevel.DEBUG.value) {
			sync.log.debug('printing status of all chunks');
			for (var chunkIndex in chunkMap) {
				var status;

				switch (chunkMap[chunkIndex]) {
					case kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_NOT_INITIATED:
						status = 'NETWORK_CALL_NOT_INITIATED';
						break;
					case kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_INITIATED:
						status = 'NETWORK_CALL_INITIATED';
						break;
					case kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_COMPLETED:
						status = 'NETWORK_CALL_COMPLETED';
						break;
					case kony.sync.CHUNKING_CONSTANTS.CHUNK_FAILED:
						status = 'CHUNK_FAILED';
						break;
					case kony.sync.CHUNKING_CONSTANTS.CHUNK_STORAGE_IN_DB_INITIATED:
						status = 'CHUNK_STORAGE_IN_DB_INITIATED';
						break;
					case kony.sync.CHUNKING_CONSTANTS.CHUNK_STORED_IN_DB:
						status = 'CHUNK_STORED_IN_DB';
						break;
					default :
						status = 'NETWORK_CALL_NOT_INITIATED';
				}
				sync.log.debug('chunk Number -' + chunkIndex + ' chunk status-' + status);
			}
		}
	}

	function isAnyChunkBeingProcessed() {
	...
	function doesAnyChunkHaveFailure() {
		var haveFailure = false;
		for (var chunkIndex in chunkMap) {
			if (chunkMap[chunkIndex] == kony.sync.CHUNKING_CONSTANTS.CHUNK_FAILED) {
				haveFailure = true;
				break;
			}
		}
	...
	return {
		...
		'setChunkStatus': setChunkStatus,
		'getChunkStatus': getChunkStatus,
		'destroy': destroyChunkMap,
		'printStatus': printStatus,
		'isAnyChunkBeingProcessed': isAnyChunkBeingProcessed,
		'doesAnyChunkHaveFailure': doesAnyChunkHaveFailure,
	};
};
...
kony.sync.downloadChunks = function(url, serviceParams, payloadId, chunkCount, hashSum, initialData, isResumed, downloadedChunks, callback){
	...
	kony.sync.initializeChunkMapper(chunkCount);
	...
	if(isResumed){
		...
		for(var i = 0; i <  downloadedChunks.length ; i++ ){
			...
			var setStatus = kony.sync.chunkMapperInstance.setChunkStatus(downloadedChunks[i], kony.sync.CHUNKING_CONSTANTS.CHUNK_STORED_IN_DB);
			sync.log.trace('setting chunk id-' + downloadedChunks[i] + ' to CHUNK_STORED_IN_DB with status as -' + setStatus);
		...
		for(callNo=1; callNo<=chunkCount; callNo++){
			if(chunkMap[callNo]===0){
				...
				var setStatus = kony.sync.chunkMapperInstance.setChunkStatus(callNo, kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_INITIATED);
				sync.log.trace('setting chunk id-' + callNo + ' to NETWORK_CALL_INITIATED with status as -' + setStatus);
	...
	function downloadNetworkCallback(result, info){
		...
		var setStatus = kony.sync.chunkMapperInstance.setChunkStatus(chunkId, kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_COMPLETED);
		sync.log.trace('setting chunk id-' + chunkId + ' to NETWORK_CALL_COMPLETED with status as -' + setStatus);

		if(isChunkPayloadIdInvalid(result)) {
			sync.log.trace(" Chunk PayloadId is Invalid : "+ payloadId);
			chunkingFailed++;
			var setStatus = kony.sync.chunkMapperInstance.setChunkStatus(chunkId, kony.sync.CHUNKING_CONSTANTS.CHUNK_FAILED);
			sync.log.trace('setting chunk id-' + chunkId + ' to CHUNK_FAILED with status as -' + setStatus);
			...
			if(callNo <= chunkCount && !kony.sync.isSyncStopped){
				if(!isResumed){
					if(callNo !== 0){
						infoObj[kony.sync.chunkNoKey] = callNo.toString();
						serviceParams[kony.sync.chunkNoKey] = callNo.toString();
			...
			if(!kony.sync.isNull(result.opstatus) && result.opstatus != 0){
				...
				//store in local DB
				var setStatus = kony.sync.chunkMapperInstance.setChunkStatus(chunkId, kony.sync.CHUNKING_CONSTANTS.CHUNK_STORAGE_IN_DB_INITIATED);
				sync.log.trace('setting chunk id-' + chunkId + ' to CHUNK_STORAGE_IN_DB_INITIATED with status as -' + setStatus);
	...
	function allChunksProcessed(){
		kony.sync.chunkMapperInstance.printStatus();
		return !kony.sync.chunkMapperInstance.isAnyChunkBeingProcessed();
	}
	...
	function handleError(){
		...
		if(chunkingFailed > 0 || kony.sync.chunkMapperInstance.doesAnyChunkHaveFailure()){
	...
	function chunkDataStoredCallback(chunkId, errorMap){
		...
		if(!kony.sync.isNull(errorMap)){
			...
			chunkProcessed++;
			var setStatus = kony.sync.chunkMapperInstance.setChunkStatus(chunkId, kony.sync.CHUNKING_CONSTANTS.CHUNK_STORED_IN_DB);
			sync.log.trace('setting chunk id-' + chunkId + ' to CHUNK_STORED_IN_DB with status as -' + setStatus);
	...
	function chunkDataClearback(data, isError){
		...
		if(isError || corruptedData){
		    // In case total download payload checksum comparison failed or clearing the chunks at local database failed,
		    // making /deletechunk call to remove chunks at sync console database
		    kony.sync.clearChunksInServerDBForPayloadId(payloadId);
			...
			try{
			    ...
			    // If parsing of total chunk data failed, making /deletechunk call to remove chunks at sync console db
				kony.sync.clearChunksInServerDBForPayloadId(payloadId);
			...
			// If enabled chunking and there are no more changes to be downloaded, making /deletechunk call.
			// This call tries to clear chunks in server database for the last batch else these will be cleared in
			// next subsequent download call
			if(!myJsonObject.d.__sync.moreChangesAvailable) {
				kony.sync.clearChunksInServerDBForPayloadId(payloadId);
			}
...
kony.sync.clearChunksInServerDBForPayloadId = function(payloadId) {
  sync.log.trace("Entering kony.sync.clearChunksInServerDBForPayloadId with payloadId : " + payloadId);
  var deleteChunkUrl = kony.sync.getDeleteChunkUrl();
  var serviceParams = {};
  kony.sync.commonServiceParams(serviceParams)
  serviceParams.deletechunkpayloadid = payloadId;

  function networkCallback(status, result) {
	sync.log.trace("Entering kony.sync.clearChunksInServerDBForPayloadId networkCallback with result : "
	                    + JSON.stringify(result) + " and status : " + status);
	if(status === 400) {
		if(!kony.sync.isNullOrUndefined(result.opstatus) && (result.opstatus != 0)) {
		    sync.log.warn("failed to clear chunks in server database for payloadId : " + payloadId);
		} else if(!kony.sync.isNullOrUndefined(result.d) && (result.d.error == true)) {
		    sync.log.warn("failed to clear chunks in server database for payloadId : " + payloadId);
		} else {
		    sync.log.trace("successfully cleared chunks in server database for payloadId : " + payloadId);
		}
	}
  }

  sync.log.trace("invoking /deletechunk call for payloadId : " + payloadId);
  kony.sync.invokeServiceAsync(deleteChunkUrl, serviceParams, networkCallback);
}
...
kony.sync.isSyncStopped = false;
kony.sync.isSyncStoppedDueToNetworkFailure = false;
...
kony.sync.CHUNKING_CONSTANTS={};
kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_NOT_INITIATED = 0;
kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_INITIATED = 1;
kony.sync.CHUNKING_CONSTANTS.NETWORK_CALL_COMPLETED = 2;
kony.sync.CHUNKING_CONSTANTS.CHUNK_FAILED = 3;
kony.sync.CHUNKING_CONSTANTS.CHUNK_STORAGE_IN_DB_INITIATED = 4;
kony.sync.CHUNKING_CONSTANTS.CHUNK_STORED_IN_DB = 5;
...
kony.sync.retryServiceCall = function(url, result, infoObj, retryCount, callback, params){
	...
	if(!kony.sync.isAppInBackground){ 
		kony.timer.schedule('syncRetryTimer' + kony.sync.currentScope + retryCount + Date.now(), retryTimerCallback, retryWait, false);
	}else{
		retryTimerCallback();
	}
...
kony.sync.checkForRetryErrorCodes = function(key){
	...
	if(!kony.sync.isNull(suppliedErrorCodes)) {
		...
		kony.sync.isSyncStopped = true;
		kony.sync.isSyncStoppedDueToNetworkFailure = true;
		return false;
	}
	else{
...
kony.sync.eligibleForRetry = function(opstatus, info){
	...
	var  noOfRetriesDefined = kony.sync.currentSyncConfigParams[kony.sync.numberOfRetriesKey];
	...
	if(!kony.sync.isNullOrUndefined(opstatus) && opstatus != 0) {
		if(kony.sync.isNull(noOfRetriesDefined)) {
		    if(!kony.sync.isNullOrUndefined(kony.sync.currentScope)) {
                kony.sync.isSyncStopped = true;
				kony.sync.isSyncStoppedDueToNetworkFailure = true;
		    }
		} else {
...
kony.sync.getDeleteChunkUrl = function() {
    var deleteChunkUrl = kony.sync.getServerURL() + "deletechunk";
    sync.log.trace("deletechunk url created -> " + deleteChunkUrl);
    return deleteChunkUrl;
}
